...
function Setting() {
  ...
  const isElectron =
    typeof window !== 'undefined' && (window as any).electron?.selectFolder;
  const [isLoadingSaveModelPathDownload, setIsLoadingSaveModelPathDownload] =
  ...
  const handleFolderSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      setFolderPath((files as any)[0].webkitRelativePath.split('/')[0]); // Get the top-level folder name
    }
  };
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      <ul className="bg-white p-4 rounded-md shadow-md">
        ...
        <li className="border-b last:border-none py-2 text-black mb-4">
          ...
          <div className="justify-end mt-4">
            <button
              className="bg-black text-white py-2 px-6 rounded-full hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              onClick={updateCachePathDownloadModel}
            >
              Save
            </button>
          </div>
