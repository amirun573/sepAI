...
from app.models.setting.setting import Get_Settings, Update_Model_Path, Update_Notification, Update_Cache_Model_Path
...
class SettingController:
    ...
    async def update_model_path(request: dict):
        ...
        if not model_download_path:
            raise HTTPException(
                status_code=400, detail="modelDownloadPath is required")
        """Fetch settings with proper error handling."""
    ...
    @router.patch("/cacheModelDownloadPath")
    async def update_model_path(request: dict):
        model_download_path = request.get("cacheModelDownloadPath")

        if not model_download_path:
            raise HTTPException(
                status_code=400, detail="cacheModelDownloadPath is required")
        """Fetch settings with proper error handling."""
        setting = await Update_Cache_Model_Path(model_download_path)
